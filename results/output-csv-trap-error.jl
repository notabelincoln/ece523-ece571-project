...
for i in x_scales
	...
	for t in test_types
		@printf("Calculating trap error for %s, %d\n", t, i);
		
		# assign the csv name
		local csv_name = @sprintf("output-test-%s-trap-%d.csv", t, i);
		...
		local avg_error = find_abs_error_mean(df.trap, 1 .- cos.(df.x));
...
f_out_name = "./compiled-perf-csv-ece523/trap-error.csv";
