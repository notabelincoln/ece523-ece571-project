...
for i in x_scales
	...
	for t in test_types
		@printf("Calculating simp error for %s, %d\n", t, i);
		
		# assign the csv name
		local csv_name = @sprintf("output-test-%s-simp-%d.csv", t, i);
		...
		local avg_error = find_abs_error_mean(df.simpsons, 1 .- cos.(df.x));
...
f_out_name = "./compiled-perf-csv-ece523/simp-error.csv";
